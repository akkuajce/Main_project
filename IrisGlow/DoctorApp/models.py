...
class Appointments(models.Model):
    ...
    canceled_by_patient = models.BooleanField(default=False)
    ...
    def is_cancellation_allowed(self):
        ...
        return time_difference.total_seconds() / 3600 > 12
    
    def save(self, *args, **kwargs):
        if self.canceled_by_patient == True:
...
from django.db import models
from .models import CustomUser  # Import the CustomUser model from your app
...
class TherapistAvailability(models.Model):
    ...
    therapist = models.ForeignKey(
        CustomUser,
        on_delete=models.CASCADE,
        related_name='availability',  # Add a related name for reverse lookup if needed
    )
